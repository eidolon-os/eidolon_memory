...
| **Config** | `config/` | `memory_settings.py`、本地 **`memory.default.yaml`**（gitignore）、模板 **`memory.default.yaml.example`**、`palace_directory.py`、管家模板 `config/prompts/`。 |
...
| 变量 | 说明 |
|------|------|
| `EIDOLON_MEMORY_SETTINGS_YAML` | 主配置文件路径；不设则 `memory.default.yaml`，缺失时用 `memory.default.yaml.example` |
...
见 [`config/memory.default.yaml.example`](config/memory.default.yaml.example)；本地复制为 **`memory.default.yaml`** 后编辑。管家默认模板在 [`config/prompts/memory_steward.md`](config/prompts/memory_steward.md)。`EIDOLON_MEMORY_SETTINGS_YAML` 可指向任意文件。
