...
### 2026-08-07：融合那一批，以及"墙"的结局

见 `MEMORY_FUSION_PLAN.md`。对本文档影响最大的一条：

**§10 里所有关于 `match_entities_for_query` 线性的记述都已经过时。** 那个读现在是索引
seek——把问题反过来问：`名字出现在句子里 ⟺ 名字等于句子的某个子串`，于是枚举句子的子串去
查，而不是枚举名字去试。语义完全等价（纯子串包含，无阈值、无候选集重验）。

后果是 `probe_wall` 在板子上从 2 万推到 30 万陈述 / 35 万实体**找不到越界点**了——35 万
实体时图占 3.89 ms / 50 ms 预算，曲线是平的。改动前在 18 万实体越界。

**所以那堵墙是实现细节，不是"保留一张图"的固有属性**，而遗忘机制差一点就被它论证成立了。
`FORGETTING_PROPOSAL.md` 的结论因此更强而不是更弱：它当时说"遗忘要按语义和字节论证，不能
按毫秒"，现在**毫秒这条彻底没有了**。

孤儿实体回收（P2 类）随之降级：它存在的唯一理由是让 `match` 变快，而 `match` 已经和实体数
无关。实体表仍然只增不减，但那从此只是**字节**问题。

同一批里还修了两处"覆盖式扫描击败非覆盖 seek"——实体 tail 索引和别名索引，后者在 7500 条
别名时是 2.83 → 0.39 ms。**同一个陷阱在同一个文件里出现两次，两次都是靠打印查询计划才看
见的**，这条比那两个索引本身更值得记住。
