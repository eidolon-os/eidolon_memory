...
## 目前最重要的一件事，以及我在它上面犯的错

前两次探针报告"40 轮产生 7 个 fragment"，我据此写下"抽取覆盖率 17.5%，是 R@5 的上限"。
**那个结论是错的**，它假设了 40 轮都被处理过，而我没有核实。

第三次带上按 turn 的计数器才看清：

```
turn_processor_decision_persisted: 7      ← 40 轮里只有 7 轮
litellm.Timeout: timeout value=30.0, time taken=93.44s
```

实测这个端点：最小调用 2.4s，真实 steward prompt（8.3k 字符）22.9s。而超时设的 30s，
余量 7s —— 一波动就超时，litellm 重试三次，正好 93s。而 turn 是**严格串行**处理的，
所以 40 轮需要约 15 分钟，bench 却只等 360s。**它超时后静默继续查询了。**

所以那个准确率测的是**等待预算，不是记忆**。抽取质量至今未知。

三处已按实测改正：`llm.timeout_seconds` 代码默认 20→90（example 里原本没有这一项，
所以新部署继承的是更紧的 20）、bench 灌入预算 360→1200s、以及**排空失败现在 exit
非 0** 而不是把警告打到 stderr 然后继续——那个静默继续正是让我读错的原因。

`FRAGMENTS_EXTRACTED` 计数器现在能区分三种情况：模型提得少 / 门槛丢弃了 / 这轮根本
没被处理。第三种就是骗了我的那个。

**站得住的结论只有延迟**：端到端过 MCP p95 104–110ms，那些查询打的是活服务。

教训写在这里而不是删掉：我从一个聚合数字推出结论，却没检查它的输入是否完整。
